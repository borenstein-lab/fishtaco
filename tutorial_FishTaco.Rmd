...
**Forum:** http://groups.google.com/forum/#!topic/fishtaco-users/
...
1) If you have not yet done so, install a Python conda environment containing the MUSiCC and FishTaco packages. To set up the environment, download and save [this .yml file](https://cnoecker.github.io/TAUMicrobiomeWorkshop2019/installs/fishtaco_1-1-1.yml) to your computer. Then run the following command in a command line shell: 
...
4) Download the example data by clicking and unzipping [this zip file](https://cnoecker.github.io/TAUMicrobiomeWorkshop2019/data/FishTacoData.zip). See below for more details on this dataset.

5) Run the FishTaco analysis on the example data:

```{bash, eval = F}
run_fishtaco.py -ta bv_qpcr.txt -fu bv_metagenomes.txt -l bv_metadata_fishtaco.txt -inf --log --assessment single_taxa
```

6) To visualize the results of the analysis in your web browser, navigate to http://elbo-spice.gs.washington.edu/shiny/FishTacoPlot, and select the following options on the right-hand menu:
...
7) Experiment with the various options under "Select Functions to Show" and "Plot Options". We recommend reducing the number of functions displayed for better readability, and selecting "Yes" for the "Add taxa names in plot" option.
...
1) Install a Python conda environment containing the MUSiCC and FishTaco packages. To set up the environment, download and save [this .yml file](https://cnoecker.github.io/TAUMicrobiomeWorkshop2019/installs/fishtaco_1-1-1.yml) to your computer. Then run the following command in a command line shell: 
...
If you have a 16S rRNA dataset consisting of samples in two different groups (e.g. cases and controls), and which you have already analyzed with PICRUSt (1 or 2), you can analyze it with FishTaco. You will need the following files: 
...
As you examine the FishTaco plots, recall that the upper bar represents case-associated taxa, while the lower bar represents control-associated taxa. Which taxa are associated with bacterial vaginosis in the example data? Which functions are determined more by shifts in the case-associated taxa, and which are determined more by the control-associated? What are the possible implications of these differences in driver taxa? 
