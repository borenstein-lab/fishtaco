...
def main(args):
    ...
    if args['normalization_mode'] == 'scale_non_permuted' and args['taxa_assessment_method'] != 'multi_taxa':
        sys.exit('Error: currently normalization_mode==scale_non_permuted can only be used with taxa_assessment_method==multi_taxa')

    if args['normalization_mode'] == 'scale_permuted' and args['taxa_assessment_method'] != 'multi_taxa' and args['taxa_assessment_method'] != 'single_taxa':
        sys.exit('Error: currently normalization_mode==scale_permuted can only be used with taxa_assessment_method==multi_taxa or taxa_assessment_method==single_taxa')
    ...
    if args['function_abun_file'] is not None:
        ...
        if 'functional_profile_already_corrected_with_musicc' in args.keys() and args['functional_profile_already_corrected_with_musicc']:
            ...
            args_for_running_musicc = {'input_file': args['function_abun_file'],
                                       'output_file': args['output_pref'] + '_STAT_function_abundance_MUSiCC_corrected' + output_suffix,
                                       'input_format': 'tab', 'output_format': 'tab', 'musicc_inter': True, 'musicc_intra': 'learn_model',
            ...
            try:
                ...
                print("Unexpected error while running MUSiCC on functional data (make sure you have KEGG orthology groups (KOs) as rows):")
                print(sys.exc_info())
                print("------------------------------------------------------------------------------------------------------------------")
                sys.exit()

            # read the corrected data into the variable
            function_abun_data = pd.read_table(args['output_pref'] + '_STAT_function_abundance_MUSiCC_corrected' + output_suffix, index_col=0)
    ...
    if args['map_function_level'] != 'none':
        ...
        if args['map_function_file'] is not None:
            if not os.path.isfile(args['map_function_file']):
                sys.exit('Error: Input file "' + args['map_function_file'] + '" does not exist')
        ...
        function_to_pathway_mapping = pd.read_table(function_mapping_file, index_col=0, dtype={0: str})

        # filter out functions from the mapping file that we don't have in our functional profile
        functions_in_mapping_with_abundance = np.intersect1d(function_to_pathway_mapping.index.values, original_function_abun_data.index.values)
        if len(functions_in_mapping_with_abundance) == 0:
            sys.exit('Error: None of the functions in the mapping file appear in the function abundance file')

        #print("function_to_pathway_mapping.shape: " + str(function_to_pathway_mapping.shape))
        function_to_pathway_mapping = function_to_pathway_mapping.loc[functions_in_mapping_with_abundance]
        #print("function_to_pathway_mapping.shape: " + str(function_to_pathway_mapping.shape))
        ...
        if args['taxa_to_function_file'] is not None:
            args_for_mapping_function_to_higher = {'ko_abun_pd': original_taxa_to_function_data,
                                                   ...
                                                   'output_pd': pd.DataFrame(),
                                                   'mapping_method': 'naive', 'compute_method': 'sum',
                                                   'transpose_ko_abundance': True, 'transpose_output': True, 'verbose': False}

            compute_pathway_abundance.main(args_for_mapping_function_to_higher)

            taxa_to_function_data = args_for_mapping_function_to_higher['output_pd']
            #print(taxa_to_function_data.to_string(max_rows=5, max_cols=5))

        #print(function_to_pathway_mapping.to_string(max_rows=5, max_cols=5))
        #print(function_abun_data.to_string(max_rows=5, max_cols=5))
        print("Done.")
    ...
    ###################################################################################################################
    # If we are given a pre-computed functional shift file, read the p-values and filter using the threshold:
    ...
    ###################################################################################################################
    # SELECT ONLY FUNCTIONS THAT ARE ENRICHED IN THE SPECIFIC SET OF SAMPLES WITH THE CASES LABEL
    ###################################################################################################################
    print("Selecting only functions that are enriched in samples with the label: " + args['case_label'] + "...")
    ...
    # for each DA function, learn a non-negative elastic-net model from taxa, using the genomic content as prior if given.
    # NOTE: We are using the elastic net only as a feature selection step, and use NNLS to fit the exact copy-number
    ###################################################################################################################
    if num_of_da_functions > 0 and 'apply_inference' in args.keys() and args['apply_inference']:

        print("Inferring the genomic content of each taxa...")

        # if we need to perform the inference on KO level, we need to identify KOs that are mapped to DA pathways, and
        # perform the inference only on them
        if args['map_function_level'] != 'none' and 'perform_inference_on_ko_level' in args.keys() and args['perform_inference_on_ko_level']:
            #print(function_to_pathway_mapping.to_string(max_cols=5, max_rows=5))
            #print(functions)
            #print(function_to_pathway_mapping.loc[:, functions].to_string(max_cols=5, max_rows=5))
            ...
            #print(functions_to_infer)
            num_of_functions_to_infer = len(functions_to_infer)
            #print(num_of_functions_to_infer)

        else:  # we are inferring content for higher level functions (e.g., pathways), or there is no mapping done at all
        ...
        all_functions_prediction_on_test_uncentered = np.zeros((number_of_samples, num_of_functions_to_infer))
        ...
        for i in range(num_of_functions_to_infer):
            ...
            # if we are inferring from KO level, then use the original functional abundance data
            if 'perform_inference_on_ko_level' in args.keys() and args['perform_inference_on_ko_level']:
                function_abun_for_curr_func = original_function_abun_data.loc[curr_function].values

            else:  # we have a mapping to a higher functional level and we are performing inference for that level
                function_abun_for_curr_func = function_abun_data.loc[curr_function].values
            ...
            if args['taxa_to_function_file'] is None:
                ...
                if 'perform_inference_on_ko_level' in args.keys() and args['perform_inference_on_ko_level']:
                    function_index_in_taxa_to_function = original_taxa_to_function_data.columns.get_loc(curr_function)
                    params = {'covariates_prior': original_taxa_to_function_data.values[:, function_index_in_taxa_to_function]}
            ...
            if np.max(params['covariates_prior']) > 0 and function_abun_non_zero > 1:
                ...
                for c in range(num_cv):
                    ...
                    #print(original_taxa_abun_data.values[non_zero_features, :].shape)
                    #print(function_abun_for_curr_func.shape)

                    # FIT A NNLS MODEL TO THE NON-ZERO FEATURES FROM THE ELASTIC NET
                    if sum(non_zero_features) > 0:
                        try:
                            nnls_weights, _ = optimize.nnls(original_taxa_abun_data.values[non_zero_features, :].T, function_abun_for_curr_func)

                        except ValueError:
                            print("Oops, an error occurred during inference!")
                            print("Number of non-zero features was: " + str(sum(non_zero_features)))
                            print("Feature values were: " + str(original_taxa_abun_data.values[non_zero_features, :].T))
                            print("Response was: " + str(function_abun_for_curr_func))
                            sys.exit()

                        model_weights[non_zero_features, c] = nnls_weights
                        #print(model_weights[non_zero_features, c])

                    #print(model_weights[:, c].shape)
                    #print(original_taxa_abun_data.values[:, test_indices].T.shape)
                ...
                #print(function_abun_for_curr_func)
            ...
            #print("True vals: " + str(function_abun_for_curr_func))

        # now, replace the original taxa-to-function data by the learned weights and continue
        if args['map_function_level'] != 'none' and 'perform_inference_on_ko_level' in args.keys() and args['perform_inference_on_ko_level']:

            original_taxa_to_function_data = pd.DataFrame(data=all_taxa_to_function_weights, index=original_taxa_abun_data.index.values, columns=functions_to_infer)
            original_taxa_to_function_data.index.name = 'Taxa'
            ...
            taxa_to_function_data = pd.DataFrame(data=all_taxa_to_function_weights, index=original_taxa_abun_data.index.values, columns=functions_to_infer)
            taxa_to_function_data.index.name = 'Taxa'
    ...
    for p in range(number_of_permutations):
        ...
        # if needed, create the permuted matrix for the residuals of all functions
        # note that if we use as_baseline, we never permute the residual, so we just create copies of the residual
        # as the permuted matrices
        if 'residual_mode' in args.keys() and args['residual_mode'] != 'remove_residual':
            for i in range(num_of_da_functions):
                if 'residual_mode' in args.keys() and args['residual_mode'] == 'as_taxa':
                    ...
                    if ['taxa_assessment_method'] == 'multi_taxa':
                        permuted_residual_matrices_function_vs_samples[p, i, :] = residual_function_vs_sample[i, :]
                    else:
                        permuted_residual_matrices_function_vs_samples[p, i, :] = np.random.permutation(residual_function_vs_sample[i, :])
    ...
    ###################################################################################################################
    # IF WE ARE NOT REMOVING THE RESIDUAL, THEN UPDATE NUM_OF_TAXA TO INCLUDE UNKNOWN
    ###################################################################################################################
    if 'residual_mode' in args.keys() and args['residual_mode'] != 'remove_residual':
        num_of_taxa += 1
    ...
    for i in range(num_of_da_functions):  # num_of_da_functions
        ...
        # create the taxa abundance data for this function, including the residual as the "unknown" taxa (if not removed)
        if 'residual_mode' in args.keys() and args['residual_mode'] != 'remove_residual':
            ...
            taxa_abun_data = pd.DataFrame(data=original_taxa_abun_data, index=original_taxa_abun_data.index.values, columns=samples)

        # now create the full permuted matrices for this function, including the residual (unless removed)
        ...
        for p in range(number_of_permutations):
            if 'residual_mode' in args.keys() and args['residual_mode'] != 'remove_residual':
                ...
                permuted_taxa_abundance_matrices[p] = no_residual_permuted_taxa_abundance_matrices[p]
        ...
        if args['taxa_assessment_method'] == 'multi_taxa':  # Shapley value analysis with permutations, not of subsets but orderings
            ...
            for p in range(number_of_permutations):
                ...
                for o in range(number_of_orderings):
                    ...
                    for s in range(num_of_taxa):
                        ...
                        #print("indexing array: " + str(indexing_array))
                        #print("non_indexed array: " + str(non_indexed_array))
                        ...
                        if args['normalization_mode'] == 'scale_non_permuted':
                            ...
                            if 'residual_mode' in args.keys() and args['residual_mode'] == 'as_baseline':
                                non_residual_indices = (non_indexed_array != (num_of_taxa-1))  # residual index is num_of_taxa-1
                            ...
                            if 'residual_mode' in args.keys() and args['residual_mode'] == 'as_baseline':
                                non_residual_indices = (indexing_array != (num_of_taxa-1))  # residual index is num_of_taxa-1
                                sum_of_non_permuted_taxa_per_sample = np.sum(curr_matrix_with_permuted_taxa[:, indexing_array[non_residual_indices]], axis=1)
                            else:  # remove_residual or as_taxa
                            ...
                            if 'residual_mode' in args.keys() and args['residual_mode'] == 'as_baseline':
                                curr_matrix_with_permuted_taxa[:, indexing_array[non_residual_indices]] = curr_matrix_with_permuted_taxa[:, indexing_array[non_residual_indices]] / (np.tile(scaling_factor_for_non_permuted, (len(indexing_array[non_residual_indices]), 1)).T)
                            else:  # remove_residual or as_taxa
                        ...
                        if args['normalization_mode'] == 'scale_permuted':
                            ...
                            if 'residual_mode' in args.keys() and args['residual_mode'] == 'as_baseline':
                                curr_matrix_with_permuted_taxa[:, non_indexed_array[non_residual_indices]] = curr_matrix_with_permuted_taxa[:, non_indexed_array[non_residual_indices]] / (np.tile(scaling_factor_for_permuted, (len(non_indexed_array[non_residual_indices]), 1)).T)
                            else:  # remove_residual or as_taxa
                            ...
                            #print(final_sum_of_curr_matrix)
            ...
            for j in range(num_of_taxa):  # num_of_taxa
                ...
                if args['taxa_assessment_method'] == 'single_taxa':  # Now permute all other taxa

                    for p in range(number_of_permutations):  # for every permuted matrix (based on the number of permutations param given)

                        # first, copy to a current matrix the permuted one, and then change only taxa j to
                        # be a non-permuted column
                        curr_matrix_with_permuted_taxa = np.copy(permuted_taxa_abundance_matrices[p])
                        ...
                        # for testing
                        #print("j=" + str(j) + " p=" + str(p) + " permuted matrix score = " + str(permuted_matrices_scores[p]))
                        #print("j=" + str(j) + " p=" + str(p) + " constructed matrix score = " + str(compute_score(constructed_abundance_values[cases], constructed_abundance_values[controls], args['score_to_compute'], args['max_score_cutoff'])))
                ...
                elif args['taxa_assessment_method'] == 'permute_all':  # For testing purposes, permute all taxa
    ...
    if num_of_da_functions > 0:
        # print out the main output file, detailing the contribution of each taxon to each functional shift
        # including the distinction between 4 modes of contribution
        ...
        cont_pd = pd.DataFrame(data=main_output_matrix, index=taxa_abun_data.index.values, columns=function_abun_data.index.values)
        ...
        with open(args['output_pref'] + '_main_output' + output_suffix, 'w') as f:
            ...
            f.write("# b:case-associated and reducing case-enrichment; " + '\n')
            f.write("# c:control-associated and driving case-enrichment; " + '\n')
            f.write("# d:control-associated and reducing case-enrichment; " + '\n')
            f.write("#" + '\n')
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=DeprecationWarning)
            cont_pd.to_csv(args['output_pref'] + '_main_output' + output_suffix, sep='\t', na_rep=args['na_rep'], encoding='utf-8', mode='a')
        ...
        # write the inferred (or original if no inference was done) genome content in each taxa
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=DeprecationWarning)
            original_taxa_to_function_data.to_csv(args['output_pref'] + '_STAT_taxa_learned_copy_num' + output_suffix, sep='\t', na_rep='None')
        ...
        if 'apply_inference' in args.keys() and args['apply_inference']:
            ...
            global_cv_pd = pd.DataFrame(data=all_functions_global_cv_test_stats, index=functions_to_infer, columns=['Global_Test_RSQR', 'Global_Test_Pearson', 'Global_Test_Spearman'])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Quantify the individual contributions of taxa to shifts observed in functional composition across different sample sets')

    # Required arguments:

    parser.add_argument('-ta', '--taxonomic_abundance_profiles', dest='taxa_abun_file', help='Input file of taxonomic abundance profiles', default=None)

    parser.add_argument('-fu', '--functional_abundance_profiles', dest='function_abun_file', help='Input file of functional abundance profiles', default=None)

    parser.add_argument('-l', '--labels', dest='class_file', help='Input file of label assignment for the two sample sets being compared', default=None)

    # Optional arguments:

    parser.add_argument('-gc', '--genomic_content_of_taxa', dest='taxa_to_function_file', help='Input file of genomic content of each taxa', default=None)

    parser.add_argument('-inf', '--perform_inference_of_genomic_content', dest='apply_inference',
                        help='Defines if genome content is inferred (either de-novo or prior-based if genomic content is also given)', action='store_true')

    parser.add_argument('-label_to_find_enrichment_in', dest='case_label', help='Define sample set label to find enrichment in (default: 1)', default='1')

    parser.add_argument('-label_to_find_enrichment_against', dest='control_label', help='Define sample set label to find enrichment against (default: 0)', default='0')

    parser.add_argument('-op', '--output_prefix', dest='output_pref', help='Output prefix for result files (default: fishtaco_out)', default='fishtaco_out')

    parser.add_argument('-map_function_level', dest='map_function_level', help='Map functions to pathways or modules (default: pathway)',
                        default='pathway', choices=['pathway', 'module', 'none', 'custom'])

    # Advanced arguments:

    parser.add_argument('-map_function_file', dest='map_function_file', help='pathways or modules mapping file (default: use internal KEGG file)',
                        default=None)

    parser.add_argument('-perform_inference_on_ko_level', dest='perform_inference_on_ko_level',
                        help='Indicates to perform the inference on the KO level (default: use the mapped functional level, e.g., pathway)', action='store_true')

    parser.add_argument('-mult_hyp', '--multiple_hypothesis_correction', dest='multiple_hypothesis_correction',
                        help='Multiple hypothesis correction for functional enrichment (default: FDR-0.05)',
                        default='FDR-0.05', choices=['Bonf', 'FDR-0.01', 'FDR-0.05', 'FDR-0.1', 'none'])

    parser.add_argument('-max_func', '--maximum_functions_to_analyze', dest='max_da_functions_cases_controls',
                        help='Maximum number of enriched functions to consider (default: All)', default=None)

    parser.add_argument('-assessment', '--taxa_assessment_method', dest='taxa_assessment_method',
                        help='The method used when assessing taxa to compute individual contributions (default: multi_taxa)',
                        default='multi_taxa', choices=['single_taxa', 'multi_taxa'])

    parser.add_argument('-score', '--score_to_compute', dest='score_to_compute',
                        help='The enrichment score to compute for each function (default: wilcoxon)',
    ...
    parser.add_argument('-number_of_shapley_orderings_per_taxa', dest='number_of_shapley_orderings_per_taxa',
                        help='number of shapley orderings per taxa (default: 5)', default='5')

    # DEPRECATED:
    #parser.add_argument('-use_gc_as_prior', '--use_genomic_content_of_taxa_as_prior', dest='use_t2f_as_prior',
    #                    help='Learn the taxa copy number of each function, using the given genomic content data as prior (default: False)', action='store_true')
    #parser.add_argument('-residual_mode', dest='residual_mode', choices=['as_taxa', 'remove_residual', 'as_baseline'],
    #                    help='How to treat the residual of the functional abundance profile (default: remove_residual)', default='remove_residual')
    #parser.add_argument('-normalization_mode', dest='normalization_mode', choices=['none', 'scale_non_permuted', 'scale_permuted'],
    #                    help='How to normalize the sample after permuting taxa (default: scale_permuted)', default='scale_permuted')
    #parser.add_argument('-permutation_mode', dest='permutation_mode', choices=['independent', 'blocks'],
    #                    help='How to permute the taxa across samples (default: blocks)', default='blocks')
